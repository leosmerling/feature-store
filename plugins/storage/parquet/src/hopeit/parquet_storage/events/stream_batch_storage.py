...
from hopeit.parquet_storage import FileStorageSettings
from hopeit.parquet_storage.partition import get_partition_key
...
async def _save_partition(partition_key: str, items: List[DataObject], context: EventContext):
    ...
    df = pd.DataFrame.from_records(
        Payload.to_obj(item) for item in items
    )
